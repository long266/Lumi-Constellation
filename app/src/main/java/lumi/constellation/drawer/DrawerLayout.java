package lumi.constellation.drawer;
...
import lumi.constellation.utils.ViewUtils;
...
public class DrawerLayout extends ToolbarLayout {
    ...
    protected void initLayoutAttrs(@Nullable AttributeSet attrs) {
        ...
        try {
            mLayout = a.getResourceId(R.styleable.ToolbarLayout_android_layout,
                    R.layout.lumi_layout_drawerlayout);
        } finally {
            a.recycle();
        }
    }
    ...
    protected void inflateChildren() {
        if (mLayout != R.layout.lumi_layout_drawerlayout) {
    ...
    private void initDrawer() {
        ...
        setNavigationButtonIcon(
                ContextCompat.getDrawable(mContext, R.drawable.ic_lumi_ab_drawer));
        setNavigationButtonTooltip(
                getResources().getText(R.string.lumi_navigation_drawer));
        ...
        mDrawer.setScrimColor(mContext.getColor(R.color.lumi_drawerlayout_drawer_dim_color));
        ...
        if (!isInEditMode()) {
            ...
            ActionBarDrawerToggle listener
                ...
                public void onDrawerSlide(View drawerView, float slideOffset) {
                    ...
                    Color.colorToHSV(mContext.getColor(R.color.lumi_round_and_bgcolor), hsv);
    ...
    public void showDrawerTopMargin(boolean show) {
        ...
        lp.topMargin = show
                ? getResources().getDimensionPixelSize(R.dimen.lumi_drawerlayout_drawer_top_margin)
                : 0;
    ...
    public void setDrawerButtonBadge(int count) {
        if (mHeaderBadge != null) {
            if (count > 0) {
                ...
                lp.width = (int) (getResources().getDimension(R.dimen.lumi_n_badge_default_width) +
                        ((float) badgeText.length() * getResources().getDimension(R.dimen.lumi_n_badge_additional_width)));
                lp.height = getResources().getDimensionPixelSize(R.dimen.lumi_n_badge_view_size);
                mHeaderBadge.setLayoutParams(lp);

                mHeaderBadge.setVisibility(View.VISIBLE);
            } else if (count == N_BADGE) {
                mHeaderBadge.setText(getResources().getString(R.string.lumi_new_badge_text));
                mHeaderBadge.setVisibility(View.VISIBLE);
            } else {
