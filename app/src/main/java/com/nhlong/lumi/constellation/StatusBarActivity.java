...
public class StatusBarActivity extends AppCompatActivity {
    private static final String LCAT = "StatusBarActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.long266_main_statusbar_activity);

        getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.settings_statusbar, new SettingsFragment())
                .commit();

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar_statusbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
		getSupportActionBar().setDisplayShowTitleEnabled(false);

        CollapsingToolbarLayout ctbl = (CollapsingToolbarLayout) findViewById(R.id.collapsing_app_bar_sb);
		ctbl.setTitle("Status Bar");
    ...
    public static class SettingsFragment extends PreferenceFragmentCompat implements OnPreferenceChangeListener, OnPreferenceClickListener {
        ...
        public boolean onPreferenceChange(Preference item, Object newValue) {
            Log.d(StatusBarActivity.LCAT, "onPreferenceChange: " + item.getClass().getSimpleName() + " " + item.getKey() + " > " + newValue);
        ...
        public boolean onPreferenceClick(Preference item) {
            ...
            if (item.getKey().startsWith("runactivity_")) {
                ...
                Log.w(StatusBarActivity.LCAT, "Run Local Activity: " + localActivity);
                ...
                Log.w(StatusBarActivity.LCAT, "Run External Activity: " + externalActivity);
                ...
                if (data.length != 2) {
                    Log.e(StatusBarActivity.LCAT, "Wrong Params Given: " + item.getKey().substring(9));
                    return false;
                }
                Log.w(StatusBarActivity.LCAT, "Run External Activity: " + data[0] + "/" + data[1]);
                ...
                if (item.getKey().substring(5).split("#").length < 1) {
                    Log.e(StatusBarActivity.LCAT, "Wrong Tool Params Given: " + item.getKey().substring(5));
                    return false;
                }
        ...
        private void dispatchItem(Preference item, Object value) {
            ...
            if (type.equals("ListPreference")) {
                ...
                Log.e(StatusBarActivity.LCAT, "Need to implement: " + type);
            }
        }
        ...
        private void dispatchLumiDetailedColorPicker(LumiDetailedColorPickerPreference item, Object value) {
            if (item != null) {
                ...
                if (key.startsWith("argb;")) {
                    ...
                    if (prefs.length != 4) {
                        Log.e(StatusBarActivity.LCAT, "color prefs wrong size! : " + prefs.length);
                        return;
                    }
                    key1 = prefs[0];
                    int val1 = Color.alpha(((Integer) value).intValue());
                    Log.d(StatusBarActivity.LCAT, "ARGBS color prefs alpha: " + key1 + " > " + val1);
                    if (key1.length() > 0) {
                        StatusBarActivity.setSettingInt(this.cr, key1, val1);
                    }
                    key2 = prefs[1];
                    int val2 = Color.red(((Integer) value).intValue());
                    Log.d(StatusBarActivity.LCAT, "ARGBS color prefs red: " + key2 + " > " + val2);
                    if (key2.length() > 0) {
                        StatusBarActivity.setSettingInt(this.cr, key2, val2);
                    }
                    key3 = prefs[2];
                    int val3 = Color.green(((Integer) value).intValue());
                    Log.d(StatusBarActivity.LCAT, "ARGBS color prefs green: " + key3 + " > " + val3);
                    if (key3.length() > 0) {
                        StatusBarActivity.setSettingInt(this.cr, key3, val3);
                    }
                    key4 = prefs[3];
                    int val4 = Color.blue(((Integer) value).intValue());
                    Log.d(StatusBarActivity.LCAT, "ARGBS color prefs blue: " + key4 + " > " + val4);
                    if (key4.length() > 0) {
                        StatusBarActivity.setSettingInt(this.cr, key4, val4);
                    }
                    ...
                    Log.d(StatusBarActivity.LCAT, "ARGBFS color prefs alpha: " + key1 + " > " + val12);
                    if (key1.length() > 0) {
                        StatusBarActivity.setSettingFloat(this.cr, key1, val12);
                    }
                    key2 = prefs[1];
                    float val22 = ((float) Color.red(((Integer) value).intValue())) / 255.0f;
                    Log.d(StatusBarActivity.LCAT, "ARGBFS color prefs red: " + key2 + " > " + val22);
                    if (key2.length() > 0) {
                        StatusBarActivity.setSettingFloat(this.cr, key2, val22);
                    }
                    key3 = prefs[2];
                    float val32 = ((float) Color.green(((Integer) value).intValue())) / 255.0f;
                    Log.d(StatusBarActivity.LCAT, "ARGBFS color prefs green: " + key3 + " > " + val32);
                    if (key3.length() > 0) {
                        StatusBarActivity.setSettingFloat(this.cr, key3, val32);
                    }
                    key4 = prefs[3];
                    float val42 = ((float) Color.blue(((Integer) value).intValue())) / 255.0f;
                    Log.d(StatusBarActivity.LCAT, "ARGBFS color prefs blue: " + key4 + " > " + val42);
                    if (key4.length() > 0) {
                        StatusBarActivity.setSettingFloat(this.cr, key4, val42);
                    }
                } else if (key.startsWith("argb_")) {
                    key = key.substring(5);
                    val = getRGB(((Integer) value).intValue(), true);
                    Log.d(StatusBarActivity.LCAT, "ARGB color prefs: " + key + " > " + val);
                    StatusBarActivity.setSettingString(this.cr, key, val);
                } else if (key.startsWith("rgb_")) {
                    key = key.substring(4);
                    val = getRGB(((Integer) value).intValue(), false);
                    Log.d(StatusBarActivity.LCAT, "RGB color prefs: " + key + " > " + val);
                    StatusBarActivity.setSettingString(this.cr, key, val);
                } else {
                    StatusBarActivity.setSettingInt(this.cr, key, ((Integer) value).intValue());
                }
            }
        }
        ...
        private void dispatchSeekBar(Preference item, Object value) {
            if (item != null) {
                StatusBarActivity.setSettingInt(this.cr, item.getKey(), ((Integer) value).intValue());
            }
        }
        ...
        private void dispatchMultiSelectList(MultiSelectListPreference item, Set<String> value) {
            if (item != null) {
                ...
                StatusBarActivity.setSettingString(this.cr, item.getKey(), val);
            }
        }

        private void dispatchText(EditTextPreference item, Object value) {
            if (item != null) {
                StatusBarActivity.setSettingString(this.cr, item.getKey(), (String) value);
            }
        }

        private void dispatchCheckbox(CheckBoxPreference item, Object value) {
            if (item != null) {
                StatusBarActivity.setSettingBoolean(this.cr, item.getKey(), ((Boolean) value).booleanValue());
            }
        }
        ...
        private void initLumiColorPicker(LumiColorPickerPreference item) {
            ...
            if (key.startsWith("argb;")) {
                ...
                setInitialColor(Color.argb(StatusBarActivity.getSettingInt(this.cr, prefs[0]), StatusBarActivity.getSettingInt(this.cr, prefs[1]), StatusBarActivity.getSettingInt(this.cr, prefs[2]), StatusBarActivity.getSettingInt(this.cr, prefs[3])));
            } else if (key.startsWith("argbf;")) {
                String[] prefs2 = key.substring(6).split(";");
                if (prefs2.length != 4) {
                    Log.e(StatusBarActivity.LCAT, "color prefs wrong size! : " + prefs2.length);
                    return;
                }
                setInitialColor(Color.argb(Integer.valueOf((int) (StatusBarActivity.getSettingFloat(this.cr, prefs2[0]) * 255.0f)).intValue(), Integer.valueOf((int) (StatusBarActivity.getSettingFloat(this.cr, prefs2[1]) * 255.0f)).intValue(), Integer.valueOf((int) (StatusBarActivity.getSettingFloat(this.cr, prefs2[2]) * 255.0f)).intValue(), Integer.valueOf((int) (StatusBarActivity.getSettingFloat(this.cr, prefs2[3]) * 255.0f)).intValue()));
            } else if (key.startsWith("argb_")) {
                String theColor = StatusBarActivity.getSettingString(this.cr, key.substring(5));
                if (theColor == null) {
                    theColor = "#FF33B5E5";
                }
                setInitialColor(Color.parseColor(theColor));
            } else if (key.startsWith("rgb_")) {
                String theColor2 = StatusBarActivity.getSettingString(this.cr, key.substring(4));
                if (theColor2 == null) {
                    theColor2 = "#33B5E5";
                }
                setInitialColor(Color.parseColor(theColor2));
            } else {
                setInitialColor(StatusBarActivity.getSettingInt(this.cr, key));
            }
        }
        ...
        private void initMultiSelectList(MultiSelectListPreference item) {
            item.setOnPreferenceChangeListener(this);
            String initial = StatusBarActivity.getSettingString(this.cr, item.getKey());
            if (initial != null) {
        ...
        private void initSwitch(SwitchPreferenceCompat item) {
            if (item != null) {
                item.setOnPreferenceChangeListener(this);
                item.setChecked(StatusBarActivity.getSettingBoolean(this.cr, item.getKey()));
            }
        }

        private void initText(EditTextPreference item) {
            item.setOnPreferenceChangeListener(this);
            item.setText(StatusBarActivity.getSettingString(this.cr, item.getKey()));
        }
        ...
        private void initList(ListPreference item) {
            item.setOnPreferenceChangeListener(this);
            item.setValue(StatusBarActivity.getSettingString(this.cr, item.getKey()));
        }
